...
function init() {
    for (var i in boardConfig.BOARDS)
        if (boardConfig.BOARDS[i].active)
...
function onSerialData(data) {
    if (config.DEBUG_BOARD) logger(data);
    data = data.replace(/[\n\r]/g, '');
    try {
    ...
    if (data) {
        ...
        if(_pauseAll && data.isPausable!=false) return;
